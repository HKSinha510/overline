...
export class NoShowCron {
    ...
    async handleGracePeriodExpirations() {
        ...
        try {
            ...
            this.logger.log(`Found ${expiredBookings.length} expired bookings. Auto-skipping to NO_SHOW.`);
    ...
    private async notifyNextInQueue(shopId: string, shopName: string) {
        try {
            ...
            this.logger.log(`Notified next customer (${customerName}) about queue advancement at shop ${shopId}`);
        } catch (error) {
